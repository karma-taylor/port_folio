...
export const PROJECTS = [
  {
    ...
    outcomes: [
      "分段冲突在保存前直接阻断，减少排班撞期与返工沟通",
      "Excel 名单锁定 + 云端同步，跨设备仍能恢复名单与工单状态",
    ],
    cover: {
    ...
    detail: {
      heroTeaser:
        "把排班冲突校验、名单导入、云端同步和规则持久化整合成一个能真实落地的排班系统。",
      focus: {
        tagline: "让“谁在哪段工期、有没有撞期”在保存前就被系统拦住，减少群里反复确认。",
        problem:
          "谁在用：施工 / 运维排班、项目统筹。痛点：同一工单多人分段参与、Excel 名单频繁变，靠人肉对齐时间窗极易撞期。",
        ownership:
          "负责需求拆解、流程建模、交互原型、冲突规则设计、名单导入策略、Supabase 数据结构设计与上线验证，推动从 Excel 协调走向系统化排班。",
        flowSummary:
          "① 月视图展示工单跨周占用 → ② 人员分段参与（非全程默认）→ ③ 以「分段」粒度做冲突校验 → ④ 云端 + 本地双持久化。",
        productJudgment:
          "没有做成自由编辑表格，而是先锁定工单、人员分段和冲突规则，让错误在保存前暴露，优先保证排班正确性。",
        promptDesign: {
      ...
      summary:
        "面向施工与运维排班场景的可视化日历工具，支持项目占用展示、人员分配、冲突校验与云端持久化，核心价值是让排班正确性在保存前就被系统保障。",
      prompt:
...

  {
    ...
    outcomes: [
      "自动抓取多源资讯并按时区定时推送，减少手工摘抄",
      "RAG 分桶 + 结构化摘要，可直接复用到邮件模板与前端页面",
    ],
    cover: {
    ...
    detail: {
      ...
      summary:
        "基于 AI Agent + RAG 思路的自动化资讯订阅平台，支持多源抓取、主题分桶、结构化摘要，并按时区定时邮件推送，适合展示我对 AI 工作流产品的拆解能力。",
      prompt:
...

  {
    ...
    outcomes: [
      ...
      "支持 Excel 批量导入导出，网页端与小程序端共用同一拆分口径",
    ],
    cover: {
      src: "./money-cover.png",
    ...
    detail: {
      ...
      summary:
        "一款基于贪心算法的薪资分配策略工具，按可用面额（50000/25000/10000/5000/1000）从大到小自动拆分张数与余数。提供网页端单笔计算与 Excel 批量导入导出；微信小程序“轻松配钞”也已上线，体现了我把规则型需求做成双端产品的能力。",
      prompt:
...

  {
    ...
    detail: {
      ...
      summary:
        "面向业务场景的汇率换算 Web 工具，支持多银行参考、固定汇率、防呆纠偏与弱网可用，重点体现我在细节交互与结果可信度上的产品判断。",
      prompt:
...
export const PROFILE = {
  ...
  description:
    "郭伟南的 AI 产品经理作品集：聚焦 AI 工作流、业务自动化、Agent 应用与真实落地案例，包括工作日历排班、HamHome 资讯日报、薪资分配策略、汇率换算 Agent 与日报自动化。",
};
